...
Sample PhongMaterial::GetSample(const IntersectionInfo& info, bool adjoint) const
{
    ...
    else // Specular bounce
    {
        ...
        auto out = Ray(info.GetPosition(), w_o.Normalized());
        if(w_i*N_g < 0 || w_o*N_g < 0 || w_i*N_s < 0 || w_o*N_s < 0)
        {
            double pdf = 0, rpdf = 0;
            return Sample(Color(0, 0, 0), out, 0, 0, false, 2);
        ...
        rpdf = pow(w_o*up, alpha)/arvo_p(N, w_o, alpha+0.01);

        Color mod = arvo_p(N, w_i, alpha)*Ks*abs(out.direction*N_s)*float(alpha + 2)/(2*F_PI);
...
double PhongMaterial::PDF(const IntersectionInfo& info, const Vector3d& out, bool adjoint, int component) const
{
    ...
    {
        Vector3d up = Reflect(-in, N_s);
        float asdf = out*up;
        return out*up > 0 ? pow(out*up, alpha)/arvo_p(N_s, -in, alpha) : 0;
    }
}
...
//        Vector3d w_o = out.direction;
//
//        if(up*out.direction < 0 || w_i*N_g < 0 || w_o*N_g < 0 || w_i*N_s < 0 || w_o*N_s < 0)
//        {
//            double pdf = 0, rpdf = 0;
//            return Sample(Color(0, 0, 0), out, 0, 0, false, 2);
//        } 
