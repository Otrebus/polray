...
Sample PhongMaterial::GetSample(const IntersectionInfo& info, bool adjoint) const
{
    ...
    else // Specular bounce
    {
        ...
        double pdf, rpdf;
        pdf = pow(w_o*up, alpha)/arvo_p(N, w_i, alpha);
        rpdf = pow(w_o*up, alpha)/arvo_p(N, w_o, alpha);

        Color mod = arvo_p(N_s, w_i, alpha)*Ks*abs(out.direction*N_s)*float(alpha + 2)/(2*F_PI);
