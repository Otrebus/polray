...
//
//    auto pdf = dir*up > 0 ? pow(dir*up, alpha)*double(alpha + 1)/(2*F_PI) : 0;
//    auto rpdf = w_i*up > 0 ? pow(w_i*up, alpha)*double(alpha + 1)/(2*F_PI) : 0;
//
//    if(pdf < 0)
//        pdf = 0;
//    if(rpdf < 0)
...
//
//    Vector3d up = N_s;
//    return out*up > 0 ? pow(out*up, alpha)*double(alpha + 1)/(2*F_PI) : 0;
//}
//
